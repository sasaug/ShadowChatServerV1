...
public class LiveBoardModule extends ANetwork implements IModule{

	public String getModuleId() {return NetworkCore.ID + ".LiveBoardModule";}

	public Type getReceiveType() {return Type.LIVEBOARD;}
	...
	public boolean onReceive(TCPClient client, byte[] data) throws Exception{	
		...
		LiveBoardMessage msg = LiveBoardMessage.parseFrom(data);
		...
		baseBuilder.setType(Type.LIVEBOARD);
		...
		if(msg.hasKeyHash() && msg.hasGroup()){
			ArrayList<String> temp = DB.getStringArray("Group", "getMembers", new String[]{msg.getGroup()});
		...
		LiveBoardMessage.Builder builder = LiveBoardMessage.newBuilder(msg);
		builder.setTimestamp(System.currentTimeMillis());
		builder.setOrigin(username);
		builder.setTarget(msg.getTarget());
		LiveBoardMessage response = builder.build();
